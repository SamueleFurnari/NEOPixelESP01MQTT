...
#pragma endregion Flame

#pragma region Circular anim

int position;
int hue = 0;

#pragma endregion Circular anim
...
void Animator::UpdateFlameColor(byte flame_num, int new_brightness)
{
    //
    uint32_t c = 0;
...
void Animator::CreateNewFlame(byte flame_num)
{
...
int Animator::GetStepSize()
{
    return random(70) + 1;
}
int Animator::GetMaxBrightness()
{
    int retVal;
...
void Animator::rotatingRainbowRing()
{
    for (int i = 0; i < CNT; i++)
...
void Animator::rotatingChangesColorHSV()
{
    for (int i = 0; i < CNT; i++)
...
void Animator::turnOn()
{
    enabled = true;
}

void Animator::turnOff()
{
    enabled = false;
    strip.clear();
    strip.show();
...
void Animator::setAnimation(Animation a)
{
    animation = a;
}

void Animator::showAnimation()
{
    switch (animation)
    {
